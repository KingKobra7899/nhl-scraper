...
def getGameString(gameid: str):
    ...
    home_team = (
        f"{data['homeTeam']['abbrev']}"
    )
    away_team = (
        f"{data['awayTeam']['abbrev']}"
    )
...
def draw_rink_features(ax, xmin=-100, xmax=100, ymin=-42.5, ymax=42.5, zone='offensive', color='white', alpha=0.3, linewidth=1.5):
    """
    ...
    if zone == 'offensive':
        ...
        blue_line_x = 25
        ax.axvline(blue_line_x, color=color, alpha=alpha, linewidth=linewidth * 1.5, linestyle='-')

        # Goal line: 89 ft from center (11 ft from end boards at x=100)
        goal_line_x = 89
        ax.axvline(goal_line_x, color=color, alpha=alpha, linewidth=linewidth, linestyle='-')
        ...
        ax.plot([goal_line_x, goal_line_x], [-crease_radius, crease_radius], color=color, alpha=alpha, linewidth=linewidth)

        # Net (4 ft deep from goal line, 6 ft wide)
        net_depth = 4
        net_width = 6
        net = plt.Rectangle((goal_line_x, -net_width/2), net_depth, net_width,
                            color=color, fill=False, alpha=alpha, linewidth=linewidth * 2)
        ax.add_patch(net)
        ...
        ax.plot(arc_x_bottom, arc_y_bottom, color=color, alpha=alpha, linewidth=linewidth)
        ...
        net_depth = 4
        net_width = 6
        net = plt.Rectangle((goal_line_x - net_depth, -net_width/2), net_depth, net_width,
                            color=color, fill=False, alpha=alpha, linewidth=linewidth * 2)
        ax.add_patch(net)
    ...
    for x, y in faceoff_positions:
        circle = plt.Circle((x, y), faceoff_radius, color=color, fill=False,
                           alpha=alpha, linewidth=linewidth)
        ax.add_patch(circle)
        # Faceoff dot (1 ft radius)
        dot = plt.Circle((x, y), 1, color=color, fill=True,
                        alpha=alpha * 1.5, linewidth=0)
        ax.add_patch(dot)
...
def plot_game_shot_density(game_id, mode='both', sigma=10):
    ...
    H1, xedges, yedges = np.histogram2d(
        team1_df['xCoord'], team1_df['yCoord'],
        bins=[100 * 3, 89 * 3],
        range=[[xmin, xmax], [ymin, ymax]]
    )
    H2, _, _ = np.histogram2d(
        team2_df['xCoord'], team2_df['yCoord'],
        bins=[100 * 3, 89 * 3],
        range=[[xmin, xmax], [ymin, ymax]]
    )
    ...
    if mode == 'both':
        ...
        cbar.set_label(f'{team1_tricode} ← → {team2_tricode}', rotation=270, labelpad=20)
